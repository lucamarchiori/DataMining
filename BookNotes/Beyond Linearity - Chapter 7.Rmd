...
customDs = data.frame(wage = ds$wage, year = ds$year, age = ds$age)
dummy_education <- model.matrix(~ education - 1, data = ds)
customDs <-cbind(customDs, dummy_education)

names(customDs)[names(customDs) == 'education1. < HS Grad'] <- 'ltHS'
names(customDs)[names(customDs) == 'education2. HS Grad'] <- 'HS'
names(customDs)[names(customDs) == 'education3. Some College'] <- 'SomeCollege'
names(customDs)[names(customDs) == 'education4. College Grad'] <- 'College'
names(customDs)[names(customDs) == 'education5. Advanced Degree'] <- 'AdvancedDegree'

caretGam <- caret::train(wage ~ . ,
                         data = customDs, 
                         method = "gam",
...
caretGam <- caret::train(wage ~ year + age + fixedEducation, data = ds, method = "gam",
                          preProcess = c('center', 'scale'), 
            family = "gaussian",  trControl = trainControl(method = "none"))
...
plot.gam(gamModel, select = 1, shade = TRUE)

# Plot the effect of year on wage
# Generate a sequence of years for plotting
years <- seq(min(ds$year), max(ds$year), by = 1)
# Predict wages for each year using the model
predicted_wages <- predict(gamModel, newdata = data.frame(year = years, age = median(ds$age)), type = "response")
# Plot the predicted wages against the years
plot(years, predicted_wages, type = "l", xlab = "Year", ylab = "Wage", main = "Predicted Wage vs. Year")


# Generate a sequence of ages for plotting
ages <- seq(min(ds$age), max(ds$age), by = 1)
# Predict wages for each age using the model
predicted_wages <- predict(gamModel, newdata = data.frame(year = median(ds$year), age = ages), type = "response")
# Plot the predicted wages against the ages
plot(ages, predicted_wages, type = "l", xlab = "Age", ylab = "Wage", main = "Predicted Wage vs. Age")
