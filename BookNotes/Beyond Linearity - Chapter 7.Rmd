...
# Fit a GAM to predict wage using smoothing splines for age (df=4) and year (df=5)
gam1 <- gam(wage ~ year, data = ds)
...
# Other GAMs
# Model1: no year + 5 df age + education
# Model2: linear year + 5 df age + education
# Model3: 4 df year + 5 df age + education

gam.m3 <- gam(wage ~ s(year, 4) + s(age, 5) + education, data = ds)
gam.m2 <- gam(wage ~ year + s(age, 5) + education, data = ds)
gam.m1 <- gam(wage ~ s(age, 5) + education, data = ds)

# Compare the models
anova(gam.m1, gam.m2, gam.m3, test = "F")

# The best model is the one with linear year and 5 df age
...
gamFormula <- formula(wage ~ year + age + education)
gam2 <- mgcv::gam(wage ~ year + age + education, family = gaussian, data = ds)

#plot.gam(gam1, all.terms = TRUE, seWithMean = TRUE)
plot(gam2)
str(gam2)
